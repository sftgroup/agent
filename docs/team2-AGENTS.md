# AGENTS.md — team2 v13.6 (EVM MCP + 7 Servers 全面升级)
...
| 操作 | 调用方式 |
|------|----------|
| 提交代码 | `git__git_push({"repo":"...","message":"..."})` |
| 推送到 GitHub | `git__git_sync({"repo":"..."})` |
| 查看状态 | `git__git_status({"repo":"..."})` |
...
### 5.3 spawn 前置：rsync 项目到 MCP 服务器
```bash
...
| security | 1 spawn | `SECURITY_REVIEW_REPORT.md` | `security-tools__contract_audit()` |
| security-check | 1 spawn | `SECURITY_SCAN_REPORT.md` | `security-tools__contract_audit()` |
...
- 环境变量: SEPOLIA_RPC_URL={xxx}, SEPOLIA_PRIVATE_KEY={xxx}
```
...
# 仅合约测试
sessions_spawn tester "只跑 CT 合约测试，读 TEST_SCENARIOS_CT.md，报告写入 E2E_TEST_REPORT.md"
```
...
| 角色 | 最终报告 |
|------|---------|
| tester | `test-reports/E2E_TEST_REPORT.md` |
| qa | `test-reports/QA_REVIEW_REPORT.md` |
| security | `test-reports/SECURITY_REVIEW_REPORT.md` |
| security-check | `test-reports/SECURITY_SCAN_REPORT.md` |
| centralized | `test-reports/SECURITY_SCAN_REPORT_CENTRALIZED.md` |
...
| 变量 | 用途 | 谁用 |
|------|------|------|
| SEPOLIA_PRIVATE_KEY | 部署者私钥 | 架构师 + tester |
| DEPLOYER | 部署者地址 | 架构师 |
| PK_3~PK_9 | 测试网私钥池 | security-check / centralized |
| SEPOLIA_RPC_URL | Sepolia Infura RPC | 所有人 |
| GITHUB_TOKEN | Git 推送 | 所有人 |
| TEST_SERVER_HOST/USER/PASSWORD | 测试服务器 | 架构师 |

> spawn 时从 `~/.bashrc` 或 `.env` 读取环境变量写入 prompt
...
| `43.156.46.187` | MCP 服务器（6 servers, 8081-9001） |
| `43.156.78.59` | 测试服务器 (3000-3999) |

登录: ubuntu / Asdf1234!

## 13. 🔑 测试账户
> 私钥 → `~/.openclaw/eth-keys.env`（11 把 ~3.17 ETH），spawn 时按需注入
...
| `~/.openclaw/eth-keys.env` | Sepolia 测试私钥池 |
