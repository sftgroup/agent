...
description: "Centralized build workflow via build. Agent must use MCP tools for all npm/docker/mobile builds."
user-invocable: true
metadata: { "openclaw": { "os": ["linux"] } }
---

# Build Operations — Centralized Build via build

> All builds must go through build. Never run `pnpm build` / `docker build` / `gradle` / `xcodebuild` directly.
...
| **Tools** | Auto-discovered via standard MCP `tools/list` |
...
|------|-----------|-----------|
| **npm** | `/tmp/build/npm-<id>/` | pnpm 9+ preferred. Falls back npm→yarn→bun |
| **docker** | `/tmp/build/docker-<id>/` | Docker BuildKit, linux/amd64+arm64 |
| **mobile** | `/tmp/build/mobile-<id>/` | Android: `ANDROID_HOME` + Gradle<br>iOS: Xcode + xcodebuild<br>Flutter: SDK in PATH<br>Expo: `EXPO_TOKEN` env |
...
- ❌ Build in project directory — use build's isolated workspace
...
| Docker push failed | Verify `registries` in `~/.build/config.json` |
