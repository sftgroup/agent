# AGENTS.md — team3 v15.0 (极致精简版 · 吸收 Team2 下沉哲学)
...
Step 0 — 调 MCP 码审工具做机械检查：
  按你的 AGENTS.md 二段式流程：先 report → 分层决策 → drill-down
...
按你的 AGENTS.md 执行完整合约安全审计：
  security-tools MCP → 威胁建模 → SCSVS 85项复查
  → {项目根目录}/test-reports/SECURITY_REVIEW_REPORT.md"
```
> 合约>5 文件时拆 2 spawn：SEC-1 自动化扫描+威胁建模 | SEC-2 SCSVS复查+CRITICAL深度

### Security-Check（合约项目补充扫描）
```
agentId="security-check", taskName="sec-scan"
task="项目根目录: {项目根目录}

按你的 AGENTS.md 执行合约安全扫描 → 标注CVE+修复版本
...
本 agent 接入中心 MCP 服务器，**禁止绕开 MCP 直接裸用 shell 命令**。
...
| 你的工作 | 必须使用 |
|---------|---------|
| **EVM 链上操作(cast/forge)** | evm-mcp |
| **代码提交/推送/同步** | git-mcp |
| **代码审查(lint/format/type)** | code-review-mcp |
| **安全审计** | security-tools-mcp |
| **构建部署(npm/docker)** | build-mcp |
| **Solana 编译/部署** | solana-mcp |

> 🔴 以上 6 类工作**严禁**通过 exec 执行对应的 shell 命令，必须走 MCP 工具调用。
> 
> ⚠️ spawn 子 agent 时，让子 agent 自己读对应的 Skill 文件获取完整用法。不要手动摘录 RPC URL、私钥、链 ID、工具名到 prompt。
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v15.0 | 07-08 | **吸收 Team2 下沉哲学**：路径体系→精简为关键报告路径表(§10)、自检→加入项目类型路由+SSH隧道(§4)、流程→文字版去掉ASCII图(§8)、参考文档→精简(§14)。保留 spawn 模板(§5)+cast(§7)。365→~280行 |
| v13.1 | 07-08 | 吸收 Team5 亮点：DO BEFORE REPLY、搜索约束、颗粒化 3-spawn、环境变量表、cast 速查 |
| v13.0 | 07-08 | 精简路由版：900→270行。移除子Agent细节。补全 DESIGN/ |
v15.0 | 07-10 | MCP 全面标准化：5 服务 84 tools，git/build/solana/code-review/security 全部 streamable-http MCP
v15.0 | 07-10 | §12 加「强制使用 MCP 场景表」：5 类工作严禁 exec，必须走对应 MCP tool
v15.0 | 07-10 | **清理本地残留**：删 cast 速查、环境变量表、测试账户表、autotest CLI；RPC/私钥/钱包改由 evm-mcp 内置；spawn 规则改为让子代理读 Skill
v15.0 | 07-10 | **下沉重构**：删所有硬编码(端口/工具数/URL)；§5 spawn模板去具体tool名→「读你自己的AGENTS.md」；§9 MCP规则去端口表→只保留强制场景映射；删§8 服务器信息(运维配置不应在agent指令里)
