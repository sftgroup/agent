...
import {
  ...
  apiSync, apiSyncStatus, apiSyncCode, apiSnapshot, apiRepoPull
...
const tools: Record<string, { handler: (input: any) => Promise<any>; description: string; schema: Record<string, string> }> = {
  ...
  repo_sync: {
    handler: apiSyncCode,
    description: "Sync code from test server to MCP local. Agent-initiated rsync over SSH.",
    schema: { team: "string", source_host: "string", source_path: "string" } as Record<string,string>,
  },
  repo_snapshot: {
    handler: apiSnapshot,
    description: "Get current snapshot SHA for a team without re-syncing. Use for code review report traceability.",
    schema: { team: "string" } as Record<string,string>,
  },
  repo_pull: {
    handler: apiRepoPull,
    description: "Pull code from test server back to MCP: rsync + git commit + push to GitHub. Reverse of repo_sync.",
    schema: { team: "string", source_host: "string", source_path: "string", message: "string (optional)", author: "string (optional)" } as Record<string,string>,
  },
};
...
app.post("/mcp", async (req, res) => {
  ...
  const send = (result: any) => res.json({ jsonrpc: "2.0", result, id });
  const err = (code: number, msg: string) => res.json({ jsonrpc: "2.0", error: { code, message: msg }, id });

  if (method === "initialize") {
    return send({ protocolVersion: "2024-11-05", serverInfo: { name: "git-mcp", version: "1.0" }, capabilities: { tools: {} } });
  }
  if (method === "tools/list") {
    const list = Object.entries(tools).map(([name, t]) => ({ name, description: t.description, inputSchema: { type: "object", properties: {} } }));
  ...
  if (method === "notifications/initialized") {
    return res.json({ jsonrpc: "2.0", id }); // no result for notifications
  }
