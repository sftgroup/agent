...
const TOOLS: Tool[] = [
  {
    name: "evm_status",
    description:
      "Query chain status: block, gas, balance, nonce, registered contracts",
    inputSchema: {
      type: "object",
  ...
  {
    name: "evm_send",
    description: "Send transaction to contract. EIP-1559 gas. Tracks in queue.",
    inputSchema: {
      type: "object",
      properties: {
        chain: { type: "string" },
  ...
  },
  {
    name: "evm_deploy",
    description:
      "Compile + deploy + auto-verify. Saves to registry. Forge + Hardhat.",
    inputSchema: {
      type: "object",
      properties: {
        chain: { type: "string" },
  ...
  {
    name: "evm_verify",
    description: "Verify deployed contract on explorer",
    inputSchema: {
      type: "object",
      properties: {
        chain: { type: "string" },
  ...
  {
    name: "evm_token",
    description: "ERC20: balance, transfer, approve, allowance",
    inputSchema: {
      type: "object",
      properties: {
        action: {
          type: "string",
          enum: ["balance", "transfer", "approve", "allowance"],
        },
        chain: { type: "string" },
        token: {
          type: "string",
          description: "Address or alias (usdc, usdt, dai, busd)",
        },
        owner: { type: "string" },
        spender: { type: "string" },
  ...
  {
    name: "evm_gas_preset",
    description: "Recommended gas per chain and priority",
    inputSchema: {
      type: "object",
      properties: {
        chain: { type: "string" },
  ...
  {
    name: "evm_registry",
    description: "Query deployment history with optional filters",
    inputSchema: {
      type: "object",
      properties: {
        chain: { type: "string" },
...
app.post("/mcp", async (req, res) => {
  ...
  if (method === "tools/call") {
    ...
    console.log(
      `[${new Date().toISOString()}] ${toolName} ${JSON.stringify(params?.arguments).slice(0, 200)}`,
    );
    try {
...
app.listen(PORT, HOST, () => {
  ...
  console.log(
    `   Auth:   ${API_TOKEN ? "Bearer token" : "OFF (set API_TOKEN to enable)"}`,
  );
});
