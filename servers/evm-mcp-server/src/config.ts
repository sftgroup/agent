...
export function getChain(chain: string): ChainConfig {
  ...
  if (!c)
    throw new Error(
      `Unknown chain: ${chain}. Known: ${Object.keys(CHAINS).join(", ")}`,
    );
...
export function resolveToken(chain: string, token: string): string {
  ...
  if (!aliases?.[lower])
    throw new Error(
      `Unknown token "${token}" on ${chain}. Use address or alias: ${Object.keys(aliases || {}).join(", ")}`,
    );
  return aliases[lower];
}

// Gas presets
const GAS_PRESETS: Record<
  string,
  Record<string, { maxFee: number; priorityFee: number }>
> = {
  eth: {
