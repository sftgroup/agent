...
export async function deploy(input: DeployInput): Promise<DeployResult> {
  ...
  const kpFile = join(
    tmpdir(),
    `solana-mcp-kp-${randomUUID().substring(0, 8)}.json`,
  );
  ...
  // Extract signature
  const sigMatch =
    /Signature: (\S+)/.exec(output) ??
    /signature: (\S+)/i.exec(output) ??
    /txid (\S+)/i.exec(output);
  const signature = sigMatch?.[1] ?? "unknown";

  // Get slot
  let slot = 0;
  try {
    const slotOut = execSync(`solana slot --url "${network}"`)
      .toString()
      .trim();
