...
export async function apiGetRepo(input: { name: string }) {
  ...
  if (!repo)
    throw new Error(
      `Repo "${input.name}" not found. Use repo_list to see available repos.`,
    );
  ...
  if (localExists) {
    try {
      unsyncedCommits = parseInt(
        gitOptional(repo.local_path, "rev-list --count origin/master..HEAD") ||
          "0",
      );
    } catch {
      unsyncedCommits = -1;
...
export async function apiCreateGithubRepo(input: {
  ...
  const githubUrl =
    ghRepo.ssh_url ?? `https://github.com/${org}/${input.name}.git`;
  const localPath = join(cfg.repoBasePath, input.name);
...
export async function apiClone(input: { name: string; branch?: string }) {
  ...
  if (existsSync(join(localPath, ".git"))) {
    ...
    logAudit(input.name, "clone", {
      branch,
      commitSha: sha,
      status: "ok",
      message: "already cloned, pulled",
    });
    return {
      alreadyCloned: true,
      path: localPath,
...
export async function apiPush(input: {
  ...
  if (!input.skipChecks) {
    ...
    if (!checkResult.passed && !input.force) {
      return {
        ...
        error:
          "Integrity check failed. Fix errors or pass force=true to override.",
  ...
  let unsyncedCount = 0;
  try {
    unsyncedCount = parseInt(
      gitOptional(localPath, "rev-list --count origin/master..HEAD") || "0",
    );
  } catch {
    unsyncedCount = -1;
  }
  ...
  db.getDb()
    ...
    .run((db.getRepo(input.name) as any)?.id ?? 0, commitSha, input.message);
...
export async function apiSync(input: {
  ...
  const unsyncedCount = parseInt(
    gitOptional(repo.local_path, `rev-list --count origin/${branch}..HEAD`) ||
      "0",
  );
  if (unsyncedCount === 0) {
    return {
  ...
  const logSummary = git(
    repo.local_path,
    `log --oneline origin/${branch}..HEAD`,
  );
  const syncCommits = logSummary
    .split("\n")
    .filter(Boolean)
    .map((line) => {
  ...
  if (input.tag) {
    ...
    createTag(
      input.name,
      input.tag,
      syncCommits[syncCommits.length - 1]?.sha ?? "",
      input.tag,
      "api",
    );
  ...
  logAudit(input.name, "sync", {
    branch,
    commitSha: headSha,
    message: `Synced ${unsyncedCount} commits`,
    status: "ok",
  });
  ...
  for (const repo of repos) {
    ...
    try {
      ...
      const count = parseInt(
        gitOptional(
          repo.local_path,
          `rev-list --count origin/${repo.default_branch}..HEAD`,
        ) || "0",
      );
      if (count > 0) {
        const log = git(
          repo.local_path,
          `log --oneline origin/${repo.default_branch}..HEAD`,
        );
        const commits = log
          .split("\n")
          .filter(Boolean)
...
export async function apiCreateTag(input: {
  ...
  git(
    repo.local_path,
    `tag -a "${input.tag}" -m "${input.description ?? input.tag}"`,
  );
  git(repo.local_path, `push origin "${input.tag}"`);

  createTag(input.name, input.tag, commitSha, input.description, "api");
  logAudit(input.name, "tag", {
    branch: input.tag,
    commitSha,
    message: `Created tag ${input.tag}`,
    status: "ok",
  });
...
export async function apiCheckout(input: { name: string; ref: string }) {
  ...
  const branch = git(repo.local_path, "branch --show-current");
  logAudit(input.name, "checkout", {
    branch,
    commitSha: gitOptional(repo.local_path, "rev-parse HEAD"),
    message: out,
  });
...
export async function apiCheck(input: { name: string; branch?: string }) {
  ...
  const results: Record<
    string,
    { passed: boolean; error?: string; detail?: string }
  > = {};
  let allPassed = true;

  if (guardConfig.checks?.checkCmd) {
    try {
      execSync(guardConfig.checks.checkCmd, {
        cwd: localPath,
        timeout: 180_000,
        maxBuffer: 5 * 1024 * 1024,
      });
      ...
      results.check = {
        passed: false,
        error: e.stderr?.toString().substring(0, 500) ?? e.message,
      };
      allPassed = false;
    }
  }
  ...
  if (guardConfig.checks?.lintCmd) {
    try {
      execSync(guardConfig.checks.lintCmd, {
        cwd: localPath,
        timeout: 120_000,
        maxBuffer: 5 * 1024 * 1024,
      });
  ...
  if (guardConfig.checks?.testCmd) {
    try {
      execSync(guardConfig.checks.testCmd, {
        cwd: localPath,
        timeout: 300_000,
  ...
  if (
    guardConfig.guardFiles &&
    Object.keys(guardConfig.guardFiles).length > 0
  ) {
    ...
    for (const [pattern, label] of Object.entries(guardConfig.guardFiles)) {
      ...
      if (matches.length > 0) {
        guardIssues.push(
          `Guard file changed: ${matches.join(", ")} [${label}] — verify intentional`,
        );
      }
    }
    ...
    results.guardFiles = {
      ...
      detail:
        guardIssues.length > 0
          ? guardIssues.join("; ")
          : "No guard files changed",
    };
    if (guardIssues.length > 0) allPassed = false;
  }

  if (
    guardConfig.contracts?.type === "solana" &&
    guardConfig.contracts.programId
  ) {
    const programIdPattern = guardConfig.contracts.programId;
    const declFiles = gitOptional(localPath, "grep -l declare_id || true")
      .split("\n")
      .filter(Boolean);
    ...
    for (const f of declFiles) {
      ...
      if (!content.includes(programIdPattern)) {
        ...
        results.contracts = {
          passed: false,
          error: `program_id changed or missing in ${f}`,
        };
      }
    }
    if (programIdOk) {
      results.contracts = {
        passed: true,
        detail: `Program ID ${programIdPattern} verified`,
      };
    } else {
      allPassed = false;
    }
...
export async function apiSyncCode(input: {
  ...
  let stdout: string;
  try {
    stdout = execSync(cmd, { timeout: 120_000, maxBuffer: 2 * 1024 * 1024 })
      .toString()
      .trim();
  } catch (e: any) {
    throw new Error(
      `rsync failed: ${e.stderr?.toString() ?? e.message}`.substring(0, 500),
    );
  }
  ...
  const fileCount =
    parseInt(
      execSync(`find ${destPath} -type f | wc -l`, { timeout: 5000 })
        .toString()
        .trim(),
    ) || 0;
  ...
  const totalBytes =
    parseInt(
      execSync(`du -sb ${destPath} | cut -f1`, { timeout: 5000 })
        .toString()
        .trim(),
    ) || 0;
